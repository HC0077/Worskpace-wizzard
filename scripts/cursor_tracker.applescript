...
-- When the application quits, close all other cursor tracker instances
on quit
    -- Close all other cursor tracker windows when this one is closed
    tell application "System Events"
        set allProcesses to every process whose name is "Cursor Tracker"
        repeat with currentProcess in allProcesses
            tell currentProcess to quit
        end repeat
    end tell
    continue quit
end quit
